...
/openspec:proposal Implement functionality to create multiple VM's with dns name resolution support
with my current stack stacks/homelab-proxmox-vm I can create a single proxmox vm with a dns name resolution.
I want to enable the additional function where I can create multiple vms with a single unified configuration.
for this I want to use a list of maps.
if possible, the existing units and modules should be extended to support both single vms and multiple vms with dns name resolution.
it should be easy to extend for additional VM properties.
the new implementation does not need to be backwards compatible. I prefer a generic approach over an approach that duplicates the modules and units.
